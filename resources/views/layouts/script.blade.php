...
<script>
    $(document).ready(function() {
        ...
        function hasTableData(tableId) {
            const table = $(`#${tableId}`);
            return table.length && table.find('tbody tr').length > 0 && !table.find('tbody tr td').first().attr('colspan');
        }

        function initializeDataTable(tableId) {
            if (!hasTableData(tableId)) return;
            ...
            $(`#${tableId}`).DataTable({
                ...
                buttons: [{
                        ...
                        customize: function(xlsx) {
                            ...
                            rows.each(function() {
                                ...
                                $('c', this).each(function() {
                                    const cellRef = $(this).attr('r');
                                    const col = cellRef.replace(/[0-9]/g, '');
                                    const row = parseInt(cellRef.replace(/[A-Z]/g, '')) + offset;
                                    $(this).attr('r', col + row);
                                });
                            });
                    ...
                    },
                    {
                        ...
                        customize: function(win) {
                            ...
                            const tglRange = `${$('#start_date').val() || '-'} s.d ${$('#end_date').val() || '-'}`;
                ...
                footerCallback: function(row, data, start, end, display) {
                    ...
                    api.column(7, {
                        ...
                        const biaya = $(cell).data('biaya') || 0;
                        totalNominal += Number(biaya);
                    });
                    $(`#${tableId} .total-biaya`).html('Rp ' + totalNominal.toLocaleString('id-ID'));
                }
            });
        }

        // Fungsi untuk menampilkan/menyembunyikan filter berdasarkan mode
        function toggleFilterInputs() {
            const selectedMode = $('#mode').val();

            // Sembunyikan semua filter dulu
            $('#filter-periode, #filter-compare-month, #filter-compare-year').hide();

            if (selectedMode === 'periode') {
                $('#filter-periode').show();
            } else if (selectedMode === 'compare_month') {
                $('#filter-compare-month').show();
            } else if (selectedMode === 'compare_year') {
                $('#filter-compare-year').show();
            }
        }
        ...
        // Panggil fungsi saat dropdown mode diganti
        $('#mode').on('change', function() {
            toggleFilterInputs();
        });
        // Inisialisasi semua tabel yang ada
        initializeDataTable('tablePemeliharaan');
        initializeDataTable('tableA');
        initializeDataTable('tableB');
...
<script>
    $(document).ready(function() {
        const tableBBM = $('#tableLaporanBBM').DataTable({
            lengthMenu: [
                [10, 25, 50, 100, -1],
                [10, 25, 50, 100, "Semua"]
            ],
            ...
            buttons: [{
                    ...
                    footer: true,
                    text: '<i class="fa-solid fa-file-excel"></i>',
                    filename: 'Laporan_BBM_' + new Date().toISOString().slice(0, 10),
                    title: null,
                    exportOptions: {
                        modifier: {
                            page: 'all'
                    ...
                    customize: function(xlsx) {
                        ...
                        // Geser semua baris data ke bawah
                        rows.each(function() {
                            const r = parseInt($(this).attr('r'));
                            $(this).attr('r', r + offset);
                            $('c', this).each(function() {
                        ...
                        // Merge cell untuk judul dan spesifikasi barang
                        let mergeCells = $('mergeCells', sheet);
                        if (mergeCells.length === 0) {
                            $('worksheet', sheet).append(
                                '<mergeCells count="2"><mergeCell ref="A1:J1"/><mergeCell ref="B5:C5"/></mergeCells>'
                        ...
                        const headerRows = `
                        <row r="1">
                            <c t="inlineStr" r="A1" s="51"><is><t>LAPORAN PEMAKAIAN BBM TAHUN ${new Date().getFullYear()}</t></is></c>
                        </row>
                        <row r="2">
                            <c t="inlineStr" r="A2"><is><t>OPD</t></is></c>
                            <c t="inlineStr" r="B2"><is><t>DINAS PEKERJAAN UMUM DAN PENATAAN RUANG KOTA TEGAL</t></is></c>
                        ...
                        <row r="3">
                            <c t="inlineStr" r="A3"><is><t>Tanggal</t></is></c>
                            <c t="inlineStr" r="B3"><is><t>${$('#start_date').val() ? new Intl.DateTimeFormat('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }).format(new Date($('#start_date').val())) : '-'} s.d ${$('#end_date').val() ? new Intl.DateTimeFormat('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }).format(new Date($('#end_date').val())) : '-'}</t></is></c>
                        </row>
                        <row r="5">
                            <c t="inlineStr" r="B5"><is><t>Spesifikasi Barang</t></is></c>
                        </row>
                        <row r="6">
                            <c t="inlineStr" r="B6"><is><t>Kode Barang</t></is></c>
                            <c t="inlineStr" r="C6"><is><t>No Register</t></is></c>
                        </row>
                    `;
                        $('sheetData row:first', sheet).before(headerRows);
                ...
                },
                {
                    ...
                    customize: function(win) {
                        ...
                        $(win.document.body).prepend(`
                        <div style="margin-bottom: 20px;">
                            <h3 style="text-align: center;">LAPORAN PEMAKAIAN BBM TAHUN ${tahun}</h3>
                            <p><strong>OPD:</strong> DINAS PEKERJAAN UMUM DAN PENATAAN RUANG KOTA TEGAL</p>
                            <p><strong>Tanggal:</strong> ${tglRange}</p>
                        </div>
                    `);
            ...
            footerCallback: function(row, data, start, end, display) {
                const api = this.api();
                const intVal = (i) => typeof i === 'string' ? i.replace(/\./g, '').replace(/[^0-9-]/g,
                ...
                api.column(9, {
                    ...
                    const nominal = $(td).data('nominal');
                    if (nominal) total += intVal(nominal);
                });

                $('#totalBiaya').html('Rp ' + total.toLocaleString('id-ID'));
            }
        });
        ...
        function filterByDate() {
            ...
            $.fn.dataTable.ext.search.pop(); // clear filter lama
            $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                const date = $(tableBBM.row(dataIndex).node()).find('td[data-tanggal]').data('tanggal');
                if (!date) return true;
                if (startDate && date < startDate) return false;
                if (endDate && date > endDate) return false;
                return true;
